...
arrayskillsMenuOption.forEach(box => {
    box.addEventListener("click", (e) => {
        ...
        if (window.innerWidth <= 600) {
            ...
            const mobileDifference = ((mobileSkillOffset - mobileSelectedOffset) / rootFontSize);
            ...
            const desktopDifference = ((currentCertPosition - skillsMenuSelectionPosition) / rootFontSize);
...
function handleLolli(array) {
    for (let i = 0; i < knowledgeLolli.length; i++) {
        let lolliLength;
        window.innerWidth <= 600 ? lolliLength = array[i] * (237/432) : lolliLength = array[i];
